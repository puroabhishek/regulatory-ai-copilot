...
def _list_control_files() -> List[str]:
    if not CONTROLS_DIR.exists():
        return []
    return sorted(p.name for p in CONTROLS_DIR.glob("*_controls.json"))


def _list_profile_files() -> List[str]:
    return sorted(list_profiles(str(PROFILES_DIR)))


def _load_selected_profile(profile_file_name: str) -> dict:
    profile_path = PROFILES_DIR / profile_file_name
...
def _read_text_file(file_bytes: bytes, file_name: str) -> str:
...
@st.cache_data(show_spinner=False)
def _read_pdf_file(file_bytes: bytes) -> str:
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "PDF support requires pypdf or PyPDF2. Install one of them first."
            ) from exc
...
@st.cache_data(show_spinner=False)
def _read_docx_file(file_bytes: bytes) -> str:
    try:
...
def _read_policy_file(file_bytes: bytes, file_name: str) -> str:
    suffix = Path(file_name).suffix.lower()

    if suffix in {".txt", ".md"}:
        return _read_text_file(file_bytes, file_name)
    if suffix == ".pdf":
        return _read_pdf_file(file_bytes)
    if suffix == ".docx":
        return _read_docx_file(file_bytes)
    if suffix == ".doc":
...
def _render_policy_input() -> str:
    ...
    if policy_file is not None:
        try:
            policy_text = _read_policy_file(policy_file.getvalue(), policy_file.name)
...
def _run_gap_analysis(
    ...
    profile_data = _load_selected_profile(profile_name)
    return run_gap_analysis_workflow(
...
def _render_gap_register(df: pd.DataFrame) -> pd.DataFrame:
    ...
    f1, f2 = st.columns(2)
    with f1:
        status_options = sorted(x for x in df["Status"].dropna().unique() if str(x).strip())
        status_filter = st.multiselect("Filter by status", options=status_options, key="gap_status_filter")
    with f2:
        severity_options = sorted(x for x in df["Severity"].dropna().unique() if str(x).strip())
    ...
    st.dataframe(
        filtered_df[[c for c in ["Control ID", "Control", "Status", "Severity", "Reason", "Remediation"] if c in filtered_df.columns]],
    ...
    with left:
        ...
        if not missing_df.empty:
            ...
            st.dataframe(
                missing_df[[c for c in ["Control ID", "Control", "Severity", "Remediation"] if c in missing_df.columns]],
                use_container_width=True,
                height=220,
            )
        if not partial_df.empty:
            ...
            st.dataframe(
                partial_df[[c for c in ["Control ID", "Control", "Severity", "Remediation"] if c in partial_df.columns]],
                use_container_width=True,
                height=220,
            )
    with right:
...
def render_gap_analyzer_tab_v2(default_model=None) -> None:
    ...
    used_control_files = result.get("used_control_files", []) or []
    new_control_files_created = result.get("new_control_files_created", []) or []
    ...
    if used_control_files:
        with st.expander("Controls used in this run"):
            st.write(used_control_files)
    if new_control_files_created:
        with st.expander("New control files created in this run"):
            st.write(new_control_files_created)

    df = _normalize_gap_rows(gap_rows)
    if df.empty:
        st.warning("Run completed, but no gap rows were returned.")
        return

    filtered_df = _render_gap_register(df)
    _render_export_section(result, gap_rows, filtered_df, run_metadata)


render_gap_analyzer_tab = render_gap_analyzer_tab_v2
